...
require_once 'requestcore.class.php'; # Bring your own HTTP class. This is mine.

# Use the Last.fm API
$fm = new LastFM();
$api_url = $fm->album->get_info(array(
	'artist' => 'Motion City Soundtrack',
	'album' => 'Even If It Kills Me'
));

# Make the HTTP request.
# This is separate from the Last.fm API
$http = new RequestCore($api_url);
$http->send_request();
...
$response->body = new SimpleXMLElement($http->get_response_body(), LIBXML_NOCDATA); # Parse the XML
$response->status = $http->get_response_code();

# Dump the response to the screen to see how to iterate over the XML nodes
print_r($response);
</pre>
